...
internal class DataStoreManagerImpl(
    ...
    override val lyricsProvider =
        settingsDataStore.data.map { preferences ->
            preferences[LYRICS_PROVIDER] ?: NOTHINGPLAYER
        }
    ...
    override val enableLiquidGlass: Flow<String>
        get() =
            settingsDataStore.data.map { preferences ->
                // Frosted/liquid glass is the default look; users who turn it off persist FALSE.
    ...
    override val glyphEnabled: Flow<String> =
        settingsDataStore.data.map { preferences ->
            preferences[GLYPH_ENABLED] ?: FALSE
        }

    override suspend fun setGlyphEnabled(enabled: Boolean) {
        withContext(Dispatchers.IO) {
            settingsDataStore.edit { settings ->
                settings[GLYPH_ENABLED] = if (enabled) TRUE else FALSE
            }
        }
    }

    override val aodLyricsColor: Flow<String> =
        settingsDataStore.data.map { preferences ->
            preferences[AOD_LYRICS_COLOR] ?: "FFFFFF"
        }

    override suspend fun setAodLyricsColor(color: String) {
        withContext(Dispatchers.IO) {
            settingsDataStore.edit { settings ->
                settings[AOD_LYRICS_COLOR] = color
            }
        }
    }

    override val aodLyricsSize: Flow<Float> =
        settingsDataStore.data.map { preferences ->
            preferences[AOD_LYRICS_SIZE] ?: 30f
        }

    override suspend fun setAodLyricsSize(size: Float) {
        withContext(Dispatchers.IO) {
            settingsDataStore.edit { settings ->
                settings[AOD_LYRICS_SIZE] = size
            }
        }
    }

    override val aodShowAlbumArt: Flow<String> =
        settingsDataStore.data.map { preferences ->
            preferences[AOD_SHOW_ALBUM_ART] ?: TRUE
        }

    override suspend fun setAodShowAlbumArt(show: Boolean) {
        withContext(Dispatchers.IO) {
            settingsDataStore.edit { settings ->
                settings[AOD_SHOW_ALBUM_ART] = if (show) TRUE else FALSE
            }
        }
    }
    ...
    override suspend fun setLyricsOffset(offsetMs: Int) {
        withContext(Dispatchers.IO) {
            settingsDataStore.edit { settings ->
    ...
    companion object Settings {
        ...
        // Nothing Glyph & AOD
        val GLYPH_ENABLED = stringPreferencesKey("glyph_enabled")
        val AOD_LYRICS_COLOR = stringPreferencesKey("aod_lyrics_color")
        val AOD_LYRICS_SIZE = floatPreferencesKey("aod_lyrics_size")
