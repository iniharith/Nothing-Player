package org.nothingplayer.lyrics.parser

import com.maxrave.domain.extension.decodeHtmlEntities
import org.nothingplayer.lyrics.domain.Lyrics
...
fun parseTtmlLyrics(data: String): Lyrics {
    ...
    for (pMatch in pRegex.findAll(data)) {
        ...
        if (spans.isNotEmpty()) {
            ...
            // YouTube TTML spans can be SYLLABLE-level ("Ka","mu","bi","sa"). The original XML
            // keeps real spaces only BETWEEN words, so use the gap between consecutive spans to
            // decide the separator: whitespace in the gap = word boundary, no gap = syllable of
            // the same word. Appending a space after every span (the old behaviour) fragmented
            // words everywhere the raw string is displayed (e.g. the home-screen widget showed
            // "Ka mu bi sa ju ga ja di kaya gu a").
            val wordParts = StringBuilder()
            for (i in spans.indices) {
                val span = spans[i]
                val word = span.groupValues[3].trim()
                if (word.isEmpty()) continue
                val beginFormatted = formatMsToLrc(parseTtmlTime(span.groupValues[1]))
                wordParts.append('<').append(beginFormatted).append('>').append(word)
                val separator =
                    if (i == spans.size - 1) {
                        " "
                    } else {
                        val gapStart = span.range.last + 1
                        val gapEnd = spans[i + 1].range.first
                        val gap =
                            if (gapEnd > gapStart) {
                                innerContent.substring(gapStart, gapEnd)
                            } else {
                                ""
                            }
                        if (gap.any { it.isWhitespace() }) " " else ""
                    }
                wordParts.append(separator)
            }
