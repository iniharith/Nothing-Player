...
class Ytmusic {
    ...
    private fun createClient() =
        HttpClient(getEngine()) {
            expectSuccess = true
            // Hard bounds on every API call. Without these a hung connection stalls the caller
            // forever — the playback resolver runs blocking on ExoPlayer's loading thread, so one
            // dead socket used to freeze song loading (and Android Auto) indefinitely.
            install(HttpTimeout) {
            ...
            install(Logging) {
                ...
                // LogLevel.ALL dumps every response body (protobuf + JSON) to the log on each
                // request; that alone added seconds to browsing. NONE keeps CurlLogger as the
                // single debug trace.
                level = LogLevel.NONE
            }
...
//                    },
    ...
    suspend fun checkForFdroidUpdate() =
        httpClient.get("https://f-droid.org/api/v1/packages/com.nothingplayer.app") {
            contentType(ContentType.Application.Json)
        }
    ...
    companion object {
        ...
        // kept in a separate repo (nothingplayer-files) so the main repo stays credential-free.
