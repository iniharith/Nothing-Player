...
/**
 * Registers the "nothingplayer://" custom URI protocol handler in Windows Registry
 * under HKEY_CURRENT_USER (no admin rights required).
 *
 * Registry structure:
 * ```
 * HKCU\Software\Classes\nothingplayer
 *     (Default) = "URL:Nothing Player Protocol"
 *     URL Protocol = ""
 *     \DefaultIcon
 *         (Default) = "\"<exe_path>\",0"
 *     \shell\open\command
...
object WindowsProtocolRegistrar {
    ...
    private const val SCHEME = "nothingplayer"

    /**
     * The Last.fm auth callback. Its scheme is fixed by the callback URL registered on the Last.fm
     * API account, so it cannot be folded into "nothingplayer".
     */
    ...
    fun register() {
        ...
        register(SCHEME, "URL:Nothing Player Protocol", exePath)
        register(LASTFM_SCHEME, "URL:Nothing Player Last.fm Callback", exePath)
    }
    ...
    private fun isAlreadyRegistered(
        ...
        return try {
            ...
            // Registry stores path with quotes: "C:\path\to\Nothing Player.exe" "%1"
    ...
    private fun resolveExePath(): String? {
        ...
        //   <app>/Nothing Player.exe
        // So we go: java.home → parent (runtime) → parent (app) → Nothing Player.exe
        ...
        if (appDir != null) {
            val exeFile = java.io.File(appDir, "NothingPlayer.exe")
            if (exeFile.exists()) {
