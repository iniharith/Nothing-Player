...
object CrashDialog {
    ...
    private fun showCrashDialog(thread: Thread, throwable: Throwable) {
        ...
        val versionInfo = try {
            "Nothing Player Desktop v${VersionManager.getVersionName()}"
        } catch (_: Exception) {
            "Nothing Player Desktop"
        }

        val dialog = JDialog().apply {
            title = "Nothing Player - Unexpected Error"
            isModal = true
        ...
        val headerPanel = JPanel(BorderLayout(8, 4)).apply {
            val titleLabel = JLabel("Nothing Player has crashed").apply {
