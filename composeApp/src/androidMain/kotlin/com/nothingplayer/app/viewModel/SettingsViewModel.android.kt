...
import com.nothingplayer.app.extension.getSizeOfFile
import com.nothingplayer.app.extension.zipInputStream
import com.nothingplayer.app.extension.zipOutputStream
...
import nothingplayer.composeapp.generated.resources.restore_success
import java.io.File
...
actual suspend fun calculateDataFraction(cacheRepository: CacheRepository): SettingsStorageSectionFraction? {
    ...
    return withContext(Dispatchers.Default) {
        ...
        if (mStorageStatsManager != null) {
            ...
            val nothingPlayerSize = getSizeOfFile(application.filesDir).bytesToMB()
            val thumbSize = (application.imageLoader.diskCache?.size ?: 0L).bytesToMB()
            val otherApp = nothingPlayerSize.let { usedSpace.minus(it) - thumbSize }
            val databaseSize =
                nothingPlayerSize - playerCache.bytesToMB() - downloadCache.bytesToMB() - canvasCache.bytesToMB()
            if (totalByte ==
                freeSpace + otherApp + nothingPlayerSize + thumbSize
            ) {
