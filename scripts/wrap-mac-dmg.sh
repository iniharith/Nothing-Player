#!/usr/bin/env bash
# wrap-mac-dmg.sh — wrap a Conveyor-built Nothing Player.app into a polished .dmg.
#
...
#    extracted `.app` in a DMG with the Nothing Player background, drag-to-
...
STAGING="$(mktemp -d -t nothingplayer-dmg-staging)"
trap 'rm -rf "$STAGING"' EXIT
cp -R "$INPUT_APP" "$STAGING/"

mkdir -p "$(dirname "$OUT_DMG")"
TMP_DMG="$(mktemp -u "$(dirname "$OUT_DMG")/.nothingplayer-final-XXXX").dmg"
RW_DMG="$(mktemp -u "$(dirname "$OUT_DMG")/.nothingplayer-rw-XXXX").dmg"

echo "[wrap-mac-dmg] create-dmg → $TMP_DMG"
create-dmg \
  --volname "Nothing Player" \
  --window-pos 200 120 \
...
# Every arch mounts a volume named "Nothing Player". The previous arch's volume may not be
# fully released yet, so a plain `hdiutil attach` on the same name fails with EBUSY (16)
# or silently remounts as "/Volumes/Nothing Player 1". Detach any stale volume first, attach
# with a retry, and use the REAL mount point hdiutil reports instead of a hardcoded path.
detach_stale_volumes() {
  for v in /Volumes/Nothing Player /Volumes/Nothing Player\ *; do
