# 0x.md - Nothing Player Project Guide for AI Agents

## 🌐 Language Rule

**Response language**: Always respond in plain **English**. Do not add translations or duplicate text in any other language.

## 📋 Project Overview

**Nothing Player** is a FOSS (Free and Open Source Software) YouTube Music client for Android and Desktop, built with Compose Multiplatform.
...
### Basic Information
- **Package name**: `com.nothingplayer.app`
...
- **lyricsService/**: Lyrics fetching (LRCLIB, Nothing Player Lyrics, BetterLyrics)
...
- **lastfm/**: direct Last.fm scrobbling for the Full build. KMP (android + jvm + ios), package `org.nothingplayer.lastfm`. Signs `api_sig` with okio's MD5; talks to `ws.audioscrobbler.com/2.0/` over form-urlencoded POST
...
- **cast-empty/**: FOSS no-op stub with identical public API (package `org.nothingplayer.cast`), keeping GMS out of F-Droid builds
...
**Location**: `composeApp/src/commonMain/kotlin/com/maxrave/nothingplayer/ui/icon/`
...
**Use it:** `SimpIcons.PlayArrow` — plus a per-icon import, `import com.nothingplayer.app.ui.icon.PlayArrow`.
...
- **Features**:
  - Deep link support (`nothingplayer://` and `nothingplayer.org`)
...
**Location**: `core/media/media-jvm/src/main/java/com/nothingplayer/media_jvm/mpv/`
...
### Translation
- Use Crowdin: https://crowdin.com/project/nothingplayer
...
### Community
- Website: https://nothingplayer.org
...
- **Nothing Player Chart**: Chart playlists integrated into Library screen
...
- **Nothing Player Lyrics voting**: Vote functionality for community lyrics
...
- **Deep link support**: `nothingplayer://` and `nothingplayer.org` URL schemes
...
## 🔄 0x.md Auto-Update Rule (MANDATORY)

After completing any of the following types of changes, the AI agent **MUST** update this 0x.md file:
...
*This document helps AI Agents quickly understand the Nothing Player project. Update regularly when there are major changes to architecture or structure.*
