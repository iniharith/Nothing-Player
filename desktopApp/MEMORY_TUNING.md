...
```bash
git -C core checkout -- media/media-jvm/src/main/java/com/nothingplayer/media_jvm/mpv/MpvPlayerAdapter.kt
rm -rf core/media/media-jvm/src/main/java/com/nothingplayer/media_jvm/memory
...
```bash
pid=$(pgrep -f -i nothingplayer | while read p; do [ "$(cat /proc/$p/comm)" = nothingplayer ] && echo $p; done | head -1)
...
```bash
pid=$(pgrep -f Nothing Player | head -1)
...
```powershell
Get-Process Nothing Player | Select-Object Name,Id,@{n="WorkingSet_MB";e={[int]($_.WorkingSet64/1MB)}}
jcmd <pid> GC.heap_info
```
